...
splits = {part.strip() for part in sys.argv[4].split(",") if part.strip()}
...
if uses_eval:
    ...
    long_eval_layout = (val_context, val_future, 0)
    short_cache_layout = (train_context, train_future, train_memory)
    if layout == long_eval_layout:
        ...
        raise SystemExit(
            ...
            f"{layout!r}; expected long eval {long_eval_layout!r} or short cached {short_cache_layout!r}"
        )
PY
