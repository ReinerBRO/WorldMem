...
if [[ "${PTM_CONTEXT_MEMORY_ONLY}" == "true" && "${PTM_VISUAL_MEMORY_SELECTION}" == "true" ]]; then
  PTM_VISUAL_NUM_CANDIDATES="${PTM_VISUAL_NUM_CANDIDATES:-64}"
  if (( PTM_MAX_HISTORY < PTM_VISUAL_NUM_CANDIDATES )); then
    PTM_MAX_HISTORY="${PTM_VISUAL_NUM_CANDIDATES}"
  fi
else
  PTM_VISUAL_NUM_CANDIDATES="${PTM_VISUAL_NUM_CANDIDATES:-${PTM_MAX_HISTORY}}"
fi
