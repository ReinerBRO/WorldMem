...
RUN_NAME="${RUN_NAME:-ptm_v4a_causal_slot_token_only_10k_$(date +%Y%m%d_%H%M%S)}"
...
export PTM_TRAIN_CONTEXT_TOKEN_SOURCE=reference_tail
...
echo "[v4] run=${RUN_NAME}"
echo "[v4] base=${PTM_BASE_CKPT}"
echo "[v4] train_cache=${PTM_NPZ_CACHE_DIR}"
echo "[v4] val_cache=${PTM_NPZ_CACHE_DIR_VAL}"
echo "[v4] clean_context_token_only=true raw_reference_length=${PTM_RAW_REFERENCE_LENGTH} memory_attention_runtime=${PTM_USE_MEMORY_ATTENTION_RUNTIME}"
