...
echo "bootstrap" > "${MARKER_PREFIX}.current_phase"
: > "${MARKER_PREFIX}.runs"
...
run_eval() {
  ...
  echo "[serial] eval ${eval_label} ckpt=${ckpt}"
  ...
  if [[ -f "${PTM_EVAL_ROOT}/generation_summary.json" ]]; then
    echo "[serial] eval summary ${PTM_EVAL_ROOT}/generation_summary.json"
...
run_train_v4a() {
  local run_name="${V4A_RUN_NAME:-ptm_v4a_causal_slot_token_only_full_dit_oasis_10k_${TS}}"
  echo "${run_name}" >> "${MARKER_PREFIX}.runs"
  echo "train ${run_name}" > "${MARKER_PREFIX}.current_phase"
  echo "[serial] train v4a full-DiT run=${run_name}"
  ...
  bash ptm/scripts/train_ptm_v4_clean_context_token_only.sh
  run_eval "${run_name}" "external_worldmem_${run_name}_eval"
}

run_train_v4b() {
  local run_name="${V4B_RUN_NAME:-ptm_v4b_context_derived_token_only_full_dit_oasis_10k_${TS}}"
  echo "${run_name}" >> "${MARKER_PREFIX}.runs"
  echo "train ${run_name}" > "${MARKER_PREFIX}.current_phase"
  echo "[serial] train v4b full-DiT run=${run_name}"
...
echo "[serial] start $(date -u +%Y-%m-%dT%H:%M:%SZ) tag=${RUN_TAG}"
echo "[serial] worldmem_cache=${WORLD_MEM_CACHE_DIR}"
echo "[serial] shard_dir=${PTMFREE_SHARD_DIR}"
echo "[serial] cuda=${CUDA_VISIBLE_DEVICES}"
